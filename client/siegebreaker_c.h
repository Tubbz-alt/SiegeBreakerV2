...
/*
 if ( src_addr[10] != '4' && src_addr[10] != '5'  )
 {
//   printf("src_addr %s dst_addr %s src_addr[10] %c \n"  ,  src_addr , dst_addr , src_addr[10] );
  fflush(stdout);
...
*/
    ...
    // printf("NEXT ACK (%ld) NEXT SEQ %ld LEN %d\n", reply_ack, reply_seq, data_length);
    //printf ("Type: %d, SSL version %x, Length of SSL payload %d\n", sslhdr->type, sslhdr->version, sslhdr->length);
    //printf ("ifrom %s:%d to %s:%d\n", src_addr, sport, dst_addr, dport);
    return sniff;
}
...
Sniff_response *decode_salsa(Sniff_response *s) {
    ...
    //int i = s20_crypt(key, S20_KEYLEN_256, nonce, 0,
    //                  some, s->length - 8);
    //if (i == S20_FAILURE) {
    //  printf("Failed to decode packet from proxy!\n");
    //  return NULL;
    //}
    free(s->msg);
    free(some);
...
int send_tls_packet(Sniff_response *resp, char *content, int sock_raw, char *tip, int length) {
    ...
    if (use_pub_priv) {
        ...
        // now decoy the connections and send the first packet
        // encrypt using salsa20 and send out
        //memcpy(encrypted, content, content_len);
        //i = s20_crypt(key, S20_KEYLEN_256, nonce, 0,
        //              encrypted, content_len);
        //if (i == S20_FAILURE) {
        //  printf("Failed to encrypt using Salsa20.\n");
        // return 0;
        //}
        memcpy(tls_start + 5 + 16, encrypted, content_len);
    }

    //create_tcp_packet (pkt_size, tls_start, resp->urg_ptr,
    // resp->window, resp->reply_seq,
    //resp->reply_ack, SSL_PORT,
    //resp->port, resp->ip, tip, 0);
